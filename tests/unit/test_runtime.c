...
TEST(value_new_range) {
  KronosValue *r1 = value_new_range(1.0, 10.0, 1.0);
  ASSERT_PTR_NOT_NULL(r1);
  ASSERT_INT_EQ(r1->type, VAL_RANGE);
  ASSERT_DOUBLE_EQ(r1->as.range.start, 1.0);
  ASSERT_DOUBLE_EQ(r1->as.range.end, 10.0);
  ASSERT_DOUBLE_EQ(r1->as.range.step, 1.0);
  ASSERT_INT_EQ(r1->refcount, 1);

  value_release(r1);
}

TEST(value_new_range_with_step) {
  KronosValue *r = value_new_range(0.0, 20.0, 5.0);
  ASSERT_PTR_NOT_NULL(r);
  ASSERT_INT_EQ(r->type, VAL_RANGE);
  ASSERT_DOUBLE_EQ(r->as.range.start, 0.0);
  ASSERT_DOUBLE_EQ(r->as.range.end, 20.0);
  ASSERT_DOUBLE_EQ(r->as.range.step, 5.0);

  value_release(r);
}

TEST(value_new_range_default_step) {
  // Step of 0.0 should default to 1.0
  KronosValue *r = value_new_range(1.0, 10.0, 0.0);
  ASSERT_PTR_NOT_NULL(r);
  ASSERT_INT_EQ(r->type, VAL_RANGE);
  ASSERT_DOUBLE_EQ(r->as.range.step, 1.0);

  value_release(r);
}

TEST(value_new_range_negative_step) {
  KronosValue *r = value_new_range(10.0, 1.0, -1.0);
  ASSERT_PTR_NOT_NULL(r);
  ASSERT_INT_EQ(r->type, VAL_RANGE);
  ASSERT_DOUBLE_EQ(r->as.range.start, 10.0);
  ASSERT_DOUBLE_EQ(r->as.range.end, 1.0);
  ASSERT_DOUBLE_EQ(r->as.range.step, -1.0);

  value_release(r);
}

TEST(value_equals_range) {
  KronosValue *r1 = value_new_range(1.0, 10.0, 1.0);
  KronosValue *r2 = value_new_range(1.0, 10.0, 1.0);
  KronosValue *r3 = value_new_range(1.0, 10.0, 2.0);
  KronosValue *r4 = value_new_range(2.0, 10.0, 1.0);

  ASSERT_TRUE(value_equals(r1, r2));
  ASSERT_FALSE(value_equals(r1, r3));
  ASSERT_FALSE(value_equals(r1, r4));
...
TEST(value_equals_range_different_types) {
  KronosValue *r = value_new_range(1.0, 10.0, 1.0);
  KronosValue *num = value_new_number(1.0);
  KronosValue *str = value_new_string("1", 1);

  ASSERT_FALSE(value_equals(r, num));
  ASSERT_FALSE(value_equals(r, str));
  ASSERT_FALSE(value_equals(num, r));
  ASSERT_FALSE(value_equals(str, r));
...
TEST(value_is_type_range) {
  KronosValue *r = value_new_range(1.0, 10.0, 1.0);

  ASSERT_TRUE(value_is_type(r, "range"));
  ASSERT_FALSE(value_is_type(r, "number"));
  ASSERT_FALSE(value_is_type(r, "string"));
  ASSERT_FALSE(value_is_type(r, "boolean"));

  value_release(r);
}

TEST(value_retain_release_range) {
  KronosValue *r = value_new_range(1.0, 10.0, 1.0);
  ASSERT_INT_EQ(r->refcount, 1);

  value_retain(r);
  ASSERT_INT_EQ(r->refcount, 2);

  value_retain(r);
  ASSERT_INT_EQ(r->refcount, 3);
...
TEST(value_print_range) {
  KronosValue *r1 = value_new_range(1.0, 10.0, 1.0);
  KronosValue *r2 = value_new_range(0.0, 20.0, 5.0);
