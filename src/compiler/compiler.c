...
Bytecode *compile(AST *ast, const char **out_err) {
  ...
  if (compiler_has_error(&c)) {
    ...
    // Note: error_message is dynamically allocated and persists for the
    // lifetime of the process (as per API contract). It is not freed here.
    ...
    while (c.loop_stack) {
      ...
      BreakContinueJump *jump = c.loop_stack->pending_jumps;
      while (jump) {
        BreakContinueJump *next_jump = jump->next;
        free(jump);
        jump = next_jump;
      }
      free(c.loop_stack);
