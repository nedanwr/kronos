...
static size_t add_constant(Compiler* c, KronosValue* value) {
    if (c->bytecode->const_count >= c->bytecode->const_capacity) {
        ...
        c->bytecode->constants = realloc(c->bytecode->constants,
...
static void compile_expression(Compiler* c, ASTNode* node) {
    ...
    switch (node->type) {
        ...
        case AST_STRING: {
            KronosValue* val = value_new_string(node->as.string.value, node->as.string.length);
            emit_constant(c, val);
            break;
        }
        
        case AST_VAR: {
            KronosValue* name = value_new_string(node->as.var_name, strlen(node->as.var_name));
...
static void compile_statement(Compiler* c, ASTNode* node) {
    ...
    switch (node->type) {
        case AST_ASSIGN: {
            ...
            // Store in variable
            KronosValue* name = value_new_string(node->as.assign.name, strlen(node->as.assign.name));
            size_t idx = add_constant(c, name);
        ...
        case AST_IF: {
            ...
            c->bytecode->code[jump_offset_pos] = (uint8_t)(jump_target - jump_offset_pos - 1);
            break;
        }
        ...
        case AST_FOR: {
            ...
            KronosValue* var_name = value_new_string(node->as.for_stmt.var, strlen(node->as.for_stmt.var));
            ...
            c->bytecode->code[exit_jump_pos] = (uint8_t)(exit_target - exit_jump_pos - 1);
            break;
        }
...
Bytecode* compile(AST* ast) {
    ...
    c.bytecode->constants = malloc(sizeof(KronosValue*) * c.bytecode->const_capacity);
