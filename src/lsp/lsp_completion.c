...
void handle_completion(const char *id, const char *body) {
  ...
  const char *builtins[][2] = {
      ...
      {"enumerate", "Pair list items with zero-based indices"},
      {"any", "Check whether any list item is truthy"},
      {"all", "Check whether all list items are truthy"},
      {"sum", "Sum a list of numbers"},
      {"find", "Find the first substring index"},
      {"rfind", "Find the last substring index"},
      {"count", "Count non-overlapping substring occurrences"},
      {"capitalize", "Uppercase the first character"},
      {"title", "Capitalize each word"},
      {"now", "Get the current Unix timestamp"},
      {"format_date", "Format a Unix timestamp"},
      {"parse_date", "Parse a formatted date"},
      {"sleep", "Pause execution for a duration"},
      {"exit", "Request interpreter exit"},
      {"args", "Get process arguments"},
      {"env", "Get an environment variable"},
      {"parse_json", "Parse JSON into Kronos values"},
      {"to_json", "Serialize a Kronos value as JSON"},
      ...
      {"match", "Check if pattern matches entire string (string, pattern)"},
      {"search", "Find first regex match in string (string, pattern)"},
      {"findall", "Find all regex matches in string (string, pattern)"},
      {"regex.match",
