...
static ASTNode* parse_assignment(Parser* p, int indent) {
    ...
    if (!consume(p, TOK_TO)) return NULL;
...
static ASTNode** parse_block(Parser* p, int parent_indent, size_t* block_size) {
    ...
    while (p->pos < p->tokens->count) {
        Token* tok = peek(p, 0);
        if (!tok || tok->type != TOK_INDENT) break;
        ...
        // More statement types will be added
...
static ASTNode* parse_for(Parser* p, int indent) {
    ...
    if (!consume(p, TOK_IN)) return NULL;
    if (!consume(p, TOK_RANGE)) return NULL;
    ...
    while (p.pos < tokens->count) {
        ...
        if (stmt) {
            if (ast->count >= ast->capacity) {
                ast->capacity *= 2;
                ast->statements = realloc(ast->statements, sizeof(ASTNode*) * ast->capacity);
            }
