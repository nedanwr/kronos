 ...
 *
 * DESIGN DECISIONS:
 * - State machine: Tokenizer uses a state machine to recognize tokens based
 *   on character sequences. Simple and efficient for the Kronos grammar.
 * - Keyword recognition: Uses perfect hash table (gperf-generated) for O(1)
 *   keyword lookup. Keywords are case-sensitive and must match exactly.
 * - Indentation tracking: Kronos uses indentation (like Python) for blocks.
 *   Tokenizer tracks indentation levels and emits INDENT/DEDENT tokens.
 * - UTF-8 support: Identifiers can contain Unicode characters (valid UTF-8
 *   sequences). This allows non-ASCII variable names while maintaining
 *   compatibility with ASCII-only code.
 * - F-string parsing: F-strings contain embedded expressions. The tokenizer
 *   emits FSTRING tokens with the raw content; the parser re-tokenizes
 *   embedded expressions (see ROADMAP.md for optimization).
 *
 * EDGE CASES:
 * - Mixed indentation: Spaces and tabs are tracked separately (mixing causes
 *   errors). Tab width is 8 spaces (TOKENIZER_TAB_WIDTH).
 * - Invalid UTF-8: Invalid sequences are rejected (identifier parsing fails).
 * - Unterminated strings: Detected and reported as errors.
 * - Number parsing: Supports integers and floats (scientific notation not
 *   currently supported).
 * - Keyword vs identifier: Keywords are reserved and cannot be used as
 ...
 *
 * DESIGN DECISION: 8 spaces per tab (POSIX standard, matches terminal
 * defaults). Mixing tabs and spaces is an error (detected separately).
 */
...
// Keyword entry structure (also defined in keywords_hash.c)
struct KeywordEntry {
  const char *keyword;
  TokenType type;
};

// Perfect hash function generated by gperf
// Forward declaration - implementation in keywords_hash.c
const struct KeywordEntry *keyword_lookup(const char *str, unsigned int len);

/**
 * @brief Determine if a string matches a Kronos keyword
 *
 * DESIGN DECISION: gperf-generated perfect hash provides O(1) lookup with no
 * collisions (better than hash table or linear search). Generated at build
 * time.
 *
 * EDGE CASES: Non-keyword returns TOK_NAME, case-sensitive, exact match
 * required.
 *
...
static TokenType match_keyword(const char *text, size_t len) {
  const struct KeywordEntry *entry = keyword_lookup(text, (unsigned int)len);
...
static bool tokenize_line(TokenArray *arr, const char *line, int indent,
  ...
  while (col < len) {
    ...
    if (line[col] == '[') {
      size_t token_col = indent + col + 1;
      Token tok = {TOK_LBRACKET, TOKEN_TEXT_LBRACKET, 1, 0, line_number,
                   token_col};
      if (!token_array_add(arr, tok, out_err, line_number, token_col)) {
        return false;
      }
